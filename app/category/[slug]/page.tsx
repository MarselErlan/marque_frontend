...
export default function CategoryPage({ params }: { params: { slug: string } }) {
      ...
      try {
        ...
        if (categorySlug) {
          try {
            ...
            // Subcategories API not available
          }
        }
      } catch (error) {
  ...
  // Load category data and products from API
  useEffect(() => {
    const loadCategory = async () => {
      try {
        ...
        // Load category products (Level 1: category -> products) with filters
  ...
  }, [params.slug, currentPage, sortBy, selectedFilters, priceRange])
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="md:hidden flex items-center gap-2 mb-3">
          ...
          <span className="text-base font-bold text-black">{category?.name || params.slug}</span>
        </div>

        {/* Title and Count */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-black">{category?.name || params.slug} <span className="text-gray-500 font-normal text-lg">{total} {t('category.products')}</span></h1>
        </div>
          ...
          {allSubcategories.length > 0 && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg hover:border-brand text-sm"
              >
                <span>{t('search.subcategory')}</span>
                <ChevronDown className="w-4 h-4" />
              </button>
              {showSubcategoryDropdown && (
                <div className="absolute left-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-20 min-w-[220px]">
                  {allSubcategories.map((subcat: any) => (
                    <Link
                      key={subcat.slug}
                      href={`/subcategory/${params.slug}/${subcat.slug}`}
      ...
      {/* All Filters Modal - Same structure as Level 2 */}
      {showAllFiltersModal && (
        <>
          ...
          <div className="md:hidden fixed inset-0 bg-white z-50 overflow-y-auto">
            ...
            <div className="px-4 py-4 space-y-4">
              ...
              {allSubcategories.length > 0 && (
                <button
                  onClick={() => setShowSubcategoryDropdown(true)}
