    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="hidden md:flex items-center gap-2 mb-4 text-sm">
          ...
          <Link 
            href={`/subcategory/${params.category}/${params.subcategory}`}
            className="text-gray-600 hover:text-brand"
          >
            {subcategory?.name || params.subcategory}
          </Link>
          <ChevronRight className="w-4 h-4 text-gray-400" />
          <span className="font-medium text-black">{secondSubcategory?.name || params.secondSubcategory}</span>
        </div>
        ...
        {/* Title and Count */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-black">{secondSubcategory?.name || params.secondSubcategory} <span className="text-gray-500 font-normal text-lg">{total} {t('category.products')}</span></h1>
        </div>
                ...
                <div className="absolute left-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-20 min-w-[200px]">
                  {allCategories.map((cat) => (
                    <button
                      key={cat.slug}
                      onClick={() => {
                        // Navigate to same subcategory under different category
                        router.push(`/subcategory/${cat.slug}/${params.subcategory}/${params.secondSubcategory}`)
      ...
      {/* All Filters Modal - Same as Level 2 page */}
      {showAllFiltersModal && (
        <>
              ...
              {secondSubcategory && (
                <div className="flex items-center justify-between py-3 border-b border-gray-100">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1 text-sm text-black"
                  >
                    {secondSubcategory.name} <ChevronRight className="w-4 h-4" />
                  </button>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    {allCategories.map((cat) => (
                      <Link 
                        key={cat.slug}
                        href={`/subcategory/${cat.slug}/${params.subcategory}/${params.secondSubcategory}`}
